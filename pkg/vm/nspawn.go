...
func (n *NspawnBackend) buildRoot(inst *Instance, cfg *config.JcardConfig) (string, error) {
	...
	files := map[string]string{
		...
		// (its home /var/empty must exist and be root-owned), plus the operator.
		"etc/passwd":        passwd,
		"etc/group":         group,
...
func resolveOperatorShell() operatorShell {
	...
	op.Username, op.Uid, op.Gid, op.Home = u.Username, u.Uid, u.Gid, u.HomeDir
	op.GroupName = "users"
...
// OperatorUsername returns the invoking user's name — the non-root user a
// container sandbox logs in as — or "root" when run as root / unresolvable.
// The daemon uses it to report the SSH login user for container backends.
func OperatorUsername() string {
	if u, err := user.Current(); err == nil && u.Uid != "0" && u.Username != "" {
		return u.Username
	}
	return "root"
}
